...
description: "Complete reference for every key in config.ini — the single configuration file that controls HANA connectivity, company databases, Service Layer credentials, and scheduler settings."
---
...
The `LeerINI(section, key)` helper method calls `GetPrivateProfileString` with a 255-character buffer and returns the trimmed string value. All reads happen through `Conexion.cs` static field initializers at class-load time, meaning any missing or malformed key will produce an empty string (or throw an `ArgumentNullException` for the mandatory `UserSL`/`PassSL` credential keys).
...
| `PassSL` | Password for `UserSL`. Appended after a colon to the serialised JSON credential before Base64 encoding. Throwing `ArgumentNullException` if absent. |
