...
If either key is missing, `CrearToken` throws immediately and the static field initialiser for that token catches the exception, logs it, and stores an empty string.
...
Inside `Intercompany.cs`, the token is attached to the HTTP request as follows (representative pattern across all sync methods):
...
public static void Fabricante(string bdDestino, string Tokendestino)
{
    ...
    request.Method = "POST";
    request.Headers["Authorization"] = Tokendestino;
